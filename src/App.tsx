...
type Panel = 'journal' | 'settings' | 'prestige' | 'reset' | 'complete' | null;
const lessons = [
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">
        <a className="nav-item selected" href="#workshop"><span>◈</span> Spark Workshop <span className="nav-number">01</span></a>
    ...
    <main id="workshop">
      ...
      <section className="page-heading"><div><div className="eyebrow">AN EXERCISE IN SMALL BEGINNINGS</div><h1>Spark Workshop<span>.</span></h1><p>A little effort. A clever machine. Something that keeps on growing.</p></div><button className="journal-button" onClick={() => setPanel('journal')}>▤ <span>Field journal</span> <span>↗</span></button></section>
      ...
      <aside className="workshop-aside"><section className="next-step"><div className="small-label">{state.mastered ? 'CHAPTER COMPLETE' : 'YOUR NEXT DISCOVERY'} <span>↗</span></div><h2>{state.mastered ? 'A brilliant beginning.' : nextGoal[0]}</h2><p>{state.mastered ? 'You made 25,000 sparks and automated every machine. Keep building, or rekindle to explore a new run.' : nextGoal[1]}</p><div className="goal-progress"><span style={{ width: `${Math.min(state.lifetime / 25000 * 100, 100)}%` }} /></div><div className="goal-caption"><span>{format(state.lifetime)} / 25K lifetime sparks</span><span>{Math.min(100, Math.floor(state.lifetime / 25000 * 100))}%</span></div></section>
        <section className="upgrades"><div className="section-heading"><h2>Fine-tune your craft</h2><span>↗</span></div>{UPGRADES.map(u => <button key={u.id} className="upgrade" disabled={state.upgrades.includes(u.id) || state.sparks < u.cost} onClick={() => act(s => upgrade(s, u.id), `${u.name} installed.`)}><span className="upgrade-symbol">{u.id === 'gloves' ? '✧' : u.id === 'wiring' ? '⌁' : '◉'}</span><span><strong>{u.name}</strong><small>{u.text}</small></span><span className="upgrade-price">{state.upgrades.includes(u.id) ? '✓' : format(u.cost)}</span></button>)}</section>
    ...
    {panel === 'prestige' && <Modal title="A brighter beginning" close={close}><p className="modal-intro">Turn this run’s experience into ember cores. Each core adds 25% to hand and machine production in every future run.</p><div className="prestige-preview"><span>You will gain</span><strong>+{reward} ◇</strong><span>New permanent bonus: +{(state.cores + reward) * 25}%</span></div><p><strong>Resets:</strong> sparks, machines, managers, upgrades, and run earnings.</p><p><strong>Keeps:</strong> ember cores, lifetime sparks, completed chapter, field journal, and settings.</p><p className="muted">Core reward = floor(√(run earnings / 10,000)). {reward === 0 ? 'Earn 10,000 sparks this run to unlock your first core.' : `The next core arrives at ${format((reward + 1) ** 2 * 10000)} run earnings.`}</p><button className="primary-button" disabled={!reward} onClick={() => { act(prestige, 'A new beginning. Your ember cores are already at work.'); close(); }}>Rekindle for {reward} cores</button></Modal>}
